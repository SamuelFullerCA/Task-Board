...
function saveProjectsToStorage(projObjs){
  localStorage.setItem("projects",JSON.stringify(projObjs));
}
...
function createProjectCard(project) {
  ...
  cardTypePara.text(project.type);
  ...
  cardDuePara.text(project.dueDate);
  ...
  cardBody.append(cardDuePara,cardTypePara,cardDeleteBtn)
...
function printProjectData() {
  ...
  for (let project of projects) {
    ...
    if(project.status==="to-do"){
      todoList.append(projCard)
    }else if(project.status==="in-progress"){
      inProgressList.append(projCard)
    } else {
      doneList.append(projCard)
    }
  }
...
function handleDeleteProject() {
  ...
  //fancier, more modern
  // let idxToDel = projects.findIndex(function(proj){
  ...
  for (let i = 0; i < projects.length; i++) {
    if(projects[i].id===projectId){
      console.log("this is the matching project!")
      console.log(projects[i])
      idxToDel = i;
    }
  }
  projects.splice(idxToDel,1);
...
//FIXME: save function has fixed name
//FIXME: projectDisplay is null pointer
//FIXME: should be .delete, not .del-project-btn